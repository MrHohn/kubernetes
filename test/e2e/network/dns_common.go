...
type resolvConfVerifier struct {
	clusterFirstConfig *runtimeapi.DNSConfig
	defaultConfig      *runtimeapi.DNSConfig
}

func (v *resolvConfVerifier) init(clusterFirstConfigStr, defaultConfigStr string) error {
	var err error

	v.defaultConfig, v.clusterFirstConfig = &runtimeapi.DNSConfig{}, &runtimeapi.DNSConfig{}
	v.clusterFirstConfig.Servers, v.clusterFirstConfig.Searches, v.clusterFirstConfig.Options, err = dns.ParseResolvConf(strings.NewReader(clusterFirstConfigStr))
	if err != nil {
		return fmt.Errorf("failed to parse resolv.conf: %v", err)
	}
	v.defaultConfig.Servers, v.defaultConfig.Searches, v.defaultConfig.Options, err = dns.ParseResolvConf(strings.NewReader(defaultConfigStr))
	if err != nil {
		return fmt.Errorf("failed to parse resolv.conf: %v", err)
	}
	return nil
...
func (v *resolvConfVerifier) verify(pod *v1.Pod, resolvConfStr string) error {
	if v.clusterFirstConfig == nil || v.defaultConfig == nil {
		return fmt.Errorf("resolvConfVerifier needs to be initialized first")
	}

	var err error
	podConfig := &runtimeapi.DNSConfig{}
	podConfig.Servers, podConfig.Searches, podConfig.Options, err = dns.ParseResolvConf(strings.NewReader(resolvConfStr))
	if err != nil {
		return fmt.Errorf("failed to parse resolv.conf: %s", resolvConfStr)
	}

	var baseConfig *runtimeapi.DNSConfig
	switch pod.Spec.DNSPolicy {
	case v1.DNSClusterFirst, v1.DNSClusterFirstWithHostNet:
		baseConfig = v.clusterFirstConfig
	case v1.DNSDefault:
		baseConfig = v.defaultConfig
	case v1.DNSNone:
		baseConfig = &runtimeapi.DNSConfig{}
	default:
		return fmt.Errorf("unrecognized dnsPolicy %s", pod.Spec.DNSPolicy)
	}
	return comparePodDNSConfigs(baseConfig, podConfig, pod.Spec.DNSConfig)
}

func comparePodDNSConfigs(baseConfig, podConfig *runtimeapi.DNSConfig, specConfig *v1.PodDNSConfig) error {
	if err := framework.SameStringArray(append(baseConfig.Servers, specConfig.Nameservers...), podConfig.Servers, false); err != nil {
		return err
	}
	if err := framework.SameStringArray(append(baseConfig.Searches, specConfig.Searches...), podConfig.Searches, false); err != nil {
		return err
	}
	mergedOptions := baseConfig.Options
	if specConfig != nil {
		mergedOptions = dns.MergeDNSOptions(baseConfig.Options, specConfig.Options)
	}
	return framework.SameStringArray(mergedOptions, podConfig.Options, false)
}
