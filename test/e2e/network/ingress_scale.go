...
import (
	...
	scale "k8s.io/kubernetes/test/e2e/network/ingressscale"
	...
	Describe("GCE [Slow] [Serial] [Feature:IngressScale]", func() {
		...
		BeforeEach(func() {
			...
			if err := scaleFramework.PrepareScaleTest(); err != nil {
				framework.Failf("Unexpect error while preraring ingress scale test: %v", err)
			}
		})
		...
		AfterEach(func() {
			if errs := scaleFramework.CleanupScaleTest(); len(errs) != 0 {
				...
				framework.Failf("Unexpect error while cleaning up ingress scale test")
			}
		})

		It("Creating and updating ingresses should happen promptly with small/medium/large amount of ingresses", func() {
			errs := scaleFramework.RunScaleTest()
			if len(errs) != 0 {
				for _, err := range errs {
					framework.Logf("%v", err)
				}
				framework.Failf("Unexpect error while running ingress scale test")
			}
